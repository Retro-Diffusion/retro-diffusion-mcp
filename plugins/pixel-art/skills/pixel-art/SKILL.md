...
1. **Pick a style**: call `list_available_styles` (filterable by model or tab). Each style declares its own size limits, batch limits, and whether it needs an input image or supports reference images. Never guess limits.
...
3. **Generate** with `create_inference`.
4. For animations or batches, prefer async generation and poll rather than blocking.
...
- Size rejected → re-check the style's limits via `list_available_styles`; each style is stricter than the global 16–512 validation.
