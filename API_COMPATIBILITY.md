...
does that internally and defaults to v1 until the server operator explicitly
opts into v2. Existing MCP installations therefore require no configuration
change.
...
- v2 is an explicit server-side opt-in.
- Rollback changes the hosted server setting back to v1; MCP clients stay
  untouched.
- No v1 redirect, deprecation header, sunset date, or removal is introduced by
  this work.
...
Any future v1 deprecation requires a separately approved lifecycle proposal and
notice period.
